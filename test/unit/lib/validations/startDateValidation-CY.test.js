...
describe('start date validation - CY', () => {
  ...
  describe('after spa', () => {
    ...
    it('should return error if date is after spa date and more than 12 months from today', () => {
      const validationResponse = validation.claimFromDateValidation(dateMoreThanOneYearInPast, oldAfterSpaDate, afterSpaKey, 'cy');
      assert.equal(validationResponse.date.text, 'Ni all y dyddiad fod yn fwy na 12 mis cyn heddiw');
      assert.equal(validationResponse.errorSummary[0].text, 'Ni all y dyddiad fod yn fwy na 12 mis cyn heddiw');
    });
  });
});
