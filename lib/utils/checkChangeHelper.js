...
const countryHelper = require('./countriesHelper');
...
const altFormatsValidation = require('../validations/altFormats');

const removeForwardSlashRegEx = /\//g;
...
module.exports = {
  ...
  idFormatter(string) {
    let formatedId = string.replace(removeForwardSlashRegEx, '');
    formatedId = formatedId.replace(/-([a-z])/g, (g) => g[1].toUpperCase());
    return formatedId;
  },
  analyticsTagFormatter(string) {
    return string.replace(removeForwardSlashRegEx, '');
  },
  ...
  setupDataAndShowErrorsMessages(req) {
    if (this.processEditSectionShowError(req)) {
      ...
      if (req.session.editSection === 'alt-formats') {
        deleteSession.deleteAltFormats(req);
        return altFormatsValidation.validate('', req.session.lang);
      }
      return false;
    }
    return false;
