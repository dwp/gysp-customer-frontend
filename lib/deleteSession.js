...
module.exports = {
  ...
  deleteInviteRequestFormData(req) {
    this.afterKey(req, 'invitation-request');
  },
  afterKey(req, key) {
    if (key === 'all') {
      this.deleteDob(req);
    ...
    if (key === 'invitation-request') {
      this.deleteRequestInvitationName(req);
      this.deleteRequestInvitationDob(req);
      this.deleteRequestInvitationAddress(req);
      this.deleteRequestInvitationAddressChoose(req);
      this.deleteRequestInvitationAddressManual(req);
    }

    if (key === 'request-invitation-name') {
      this.deleteRequestInvitationDob(req);
      this.deleteRequestInvitationAddress(req);
      this.deleteRequestInvitationAddressChoose(req);
      this.deleteRequestInvitationAddressManual(req);
    ...
    if (key === 'request-invitation-dob') {
      this.deleteRequestInvitationAddress(req);
      this.deleteRequestInvitationAddressChoose(req);
      this.deleteRequestInvitationAddressManual(req);
    }

    if (key === 'request-invitation-address') {
      this.deleteRequestInvitationAddressChoose(req);
      this.deleteRequestInvitationAddressManual(req);
    }

    if (key === 'request-invitation-address-choose') {
      this.deleteRequestInvitationAddressManual(req);
    }
  },
  ...
  deleteAccountDetails(req) {
    ...
    safeDelete(req, 'accountDetailsEntered');
  },
  ...
  deleteSystemData(req) {
    ...
    safeDelete(req, 'userInInviteRequest');
    safeDelete(req, 'userHasCompletedInviteRequest');
    safeDelete(req, 'userHasCompletedInviteRequestRedirectUrl');
  },
  ...
  deleteRequestInvitationName(req) {
    safeDelete(req, 'request-invitation-name');
  },
  deleteRequestInvitationDob(req) {
    safeDelete(req, 'newStatePensionDate');
    safeDelete(req, 'request-invitation-dob');
  },
  deleteRequestInvitationAddress(req) {
    safeDelete(req, 'request-invitation-address');
  },
  deleteRequestInvitationAddressChoose(req) {
    safeDelete(req, 'addressLookup');
    safeDelete(req, 'request-invitation-address-choose');
  },
  deleteRequestInvitationAddressManual(req) {
    safeDelete(req, 'request-invitation-address-manual');
  },
  ...
  destroySessionExcludingLanguage(req) {
    this.afterKey(req, 'invitation-request');
    this.afterKey(req, 'all');
    this.deleteDob(req);
    this.deleteClaimFromData(req);
  ...
  destroyInviteRequestSessionExcludingLanguage(req) {
    this.afterKey(req, 'invitation-request');
    this.deleteSystemData(req);
  },
};
