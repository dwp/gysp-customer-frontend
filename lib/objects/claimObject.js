...
const generalHelper = require('../validations/utils/general');
const countryHelper = require('../utils/countriesHelper');
const dateFormatter = require('../helpers/dateFormatter');
const { cleanSortCode } = require('../utils/sortCode');
...
const altFormatsChoice = (details) => {
  if (details['alt-formats'] && details['alt-formats'] === 'yes') {
    const i18nForEnglishOnly = i18n.cloneInstance();
    i18nForEnglishOnly.changeLanguage('en');
    return {
      altFormatToUse: i18nForEnglishOnly.t(`alt-formats-choose:fields.${details['alt-formats-choose']}`),
      altFormatToUseQ: i18n.t('alt-formats-choose:header'),
    };
  }
  return {};
};

const buildAccountDetailsObj = (accountDetails, accountStatus) => {
  const account = {};
  ...
  account.accountHolderQ = i18n.t('account:fields.accountHolder.previousLabel');
  account.accountNumber = accountDetails.bankAccountNumber;
  account.accountNumberQ = i18n.t('account:fields.accountNumber.label');
  account.sortCode = cleanSortCode(accountDetails.bankSortCode);
  ...
  if (!generalHelper.isThisUndefinedOrEmtpy(accountDetails.buildingRoll)) {
    ...
    account.referenceNumberQ = i18n.t('account:fields.buildingRoll.previousLabel');
  } else {
    account.validated = accountDetails.validated;
    const isKbvFlagPresent = accountDetails.kbvFlag !== undefined;
    if (isKbvFlagPresent) {
      account.kbvFlag = accountDetails.kbvFlag;
    }
  }
  return account;
};
...
module.exports = {
  createCustomerObject(customerDetails) {
    ...
    const CustomerObject = { ...customerDetails, ...customerQuestions };
  ...
  accountDetailsTransformer(accountDetails, accountStatus) {
    ...
    if (accountDetails.paymentMethod === 'bank') {
      details = {
        bankDetail: buildAccountDetailsObj(accountDetails, accountStatus),
        bankDetailQ: i18n.t('account:fields.paymentMethod.options.bank'),
        paymentMethodQ: i18n.t('account:previousHeader'),
      };
    } else {
      details = {
        buildingSocietyDetail: buildAccountDetailsObj(accountDetails, accountStatus),
  ...
  async sessionToObject(details, accountStatus, language) {
    ...
    const json = {
      ...
      altFormatRequiredQ: i18n.t('alt-formats:header'),
      altFormatRequired: details['alt-formats'] === 'yes',
      ...altFormatsChoice(details),
    };
    ...
    if (!generalHelper.isThisUndefinedOrEmtpy(details.prison)
      && !generalHelper.isThisUndefinedOrEmtpy(details.prison.spentTimeInPrison)) {
      json.prison = false;
      if (details.prison.spentTimeInPrison === 'yes') {
        json.prison = true;
      }
      const { statePensionDate } = details.customerDetails;
      const date = dateFormatter.statePensionDate(statePensionDate, language);
      json.prisonQ = i18n.t('prison:fields.spentTimeInPrison.legend', { DATE: date });
    }
    ...
    if (details.isOverseas) {
      ...
      json.accountDetailQ = i18n.t('account:previousHeader');
    }
