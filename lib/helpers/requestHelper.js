...
const frontendApikey = encyption.decrypt(config.application.urls.claimServiceApiKey, config.secret);
...
module.exports = {
  ...
  errorLoggingHelper(error, location, traceID, logger, inviteKey) {
    ...
    if (error.statusCode && headerErrorMessage) {
      logger.error({ inviteKey, traceID }, `${error.statusCode} - ${headerErrorMessage} - Requested on ${location}`);
  ...
  infoLoggingHelper(error, location, traceID, logger, inviteKey) {
    const headerErrorMessage = getResponseHeaderMessage(error);
    if (error.statusCode && headerErrorMessage) {
      logger.info({ inviteKey, traceID }, `${error.statusCode} - ${headerErrorMessage} - Requested on ${location}`);
    } else if (error.statusCode) {
      logger.info({ inviteKey, traceID }, `${error.statusCode} - ${error.message} - Requested on ${location}`);
    } else {
